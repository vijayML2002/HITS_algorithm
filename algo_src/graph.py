...
class Node:
    ...
    def update_hub_value(self, weights):
        new_hub_value = 0
        for node in self.children:
            new_hub_value += (weights[self.name][node.name] * node.auth_value)
        self.hub_value = new_hub_value
        
    def update_auth_value(self, weights):
        new_auth_value = 0
        for node in self.parents:
            new_auth_value += (weights[node.name][self.name] * node.hub_value)
